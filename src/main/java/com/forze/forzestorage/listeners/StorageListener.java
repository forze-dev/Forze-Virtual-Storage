...
public class StorageListener implements Listener {
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        // ВИПРАВЛЕННЯ: Використовуємо правильний метод отримання GUI
    ...
    public void onInventoryClose(InventoryCloseEvent event) {
        ...
        StorageGUI gui = getStorageGUI(player);
        if (gui != null) {
            // ВИПРАВЛЕННЯ: Зберігаємо тільки якщо не відбувається навігація
            if (!gui.isNavigating()) {
                gui.saveStorage();
                plugin.getLogger().info("Збережено сховище при закритті для " + player.getName());
            } else {
                plugin.getLogger().info("Пропущено збереження - відбувається навігація");
            }
        }
    ...
    public void onPlayerQuit(PlayerQuitEvent event) {
        ...
        try {
            // ВИПРАВЛЕННЯ: Використовуємо правильний метод отримання GUI
            StorageGUI gui = getStorageGUI(player);
            if (gui != null && !gui.isNavigating()) {
                gui.saveStorage();
                plugin.getLogger().info("Збережено сховище при виході для " + player.getName());
            } else if (gui != null && gui.isNavigating()) {
                plugin.getLogger().warning("Гравець " + player.getName() + " вийшов під час навігації - збереження пропущено");
            }
        } catch (Exception e) {
    ...
    // ВИПРАВЛЕННЯ: Переписаний метод отримання GUI
    private StorageGUI getStorageGUI(Player player) {
        UUID uuid = player.getUniqueId();
        StorageGUI gui = plugin.getStorageManager().getOpenStorage(uuid);
        
        if (gui == null) {
            plugin.getLogger().warning("GUI не знайдено для " + player.getName());
            return null;
        }
    ...
    // ВИПРАВЛЕННЯ: Покращена обробка навігаційних кліків
    private void handleNavigationClick(InventoryClickEvent event, Player player, StorageGUI gui) {
        ...
        // ВИПРАВЛЕННЯ: Подвійний захист від множинних кліків
        if (navigatingPlayers.contains(playerId) || gui.isNavigating()) {
        ...
        // ВИПРАВЛЕННЯ: Виконуємо навігацію без затримки, але з захистом
        try {
            if (slot == 45) {
                // Попередня сторінка
            ...
            // Знімаємо захист через більше тіків для гарантії
