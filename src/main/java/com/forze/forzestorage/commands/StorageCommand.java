...
public class StorageCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "reload":
                handleReload(sender);
                break;
            // ВИПРАВЛЕННЯ: Додано команду діагностики
            case "debug":
                handleDebug(sender, args);
                break;
            // ВИПРАВЛЕННЯ: Додано команду статистики
            case "stats":
                handleStats(sender);
                break;
            default:
    ...
    private void handleDebug(CommandSender sender, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length == 1) {
                // Діагностика власного сховища
                StorageGUI gui = plugin.getStorageManager().getOpenStorage(player.getUniqueId());
                if (gui != null) {
                    gui.debugCurrentState();
                    player.sendMessage(ChatColor.GREEN + "✓ Діагностика GUI виведена в консоль");
                } else {
                    player.sendMessage(ChatColor.YELLOW + "Сховище не відкрито");
                }

                // Додаткова діагностика файлу сховища
                plugin.getStorageManager().debugStorage(player.getUniqueId());
                player.sendMessage(ChatColor.GREEN + "✓ Діагностика файлу сховища виведена в консоль");

            } else if (args.length == 2) {
                // Діагностика сховища іншого гравця
                OfflinePlayer target = Bukkit.getOfflinePlayer(args[1]);
                if (target == null || (!target.hasPlayedBefore() && !target.isOnline())) {
                    player.sendMessage(getMessage("player-not-found"));
                    return;
                ...
                plugin.getStorageManager().debugStorage(target.getUniqueId());
                player.sendMessage(ChatColor.GREEN + "✓ Діагностика сховища гравця " + target.getName() + " виведена в консоль");
            }
        } else {
            // Діагностика з консолі
            if (args.length >= 2) {
                OfflinePlayer target = Bukkit.getOfflinePlayer(args[1]);
                if (target != null && (target.hasPlayedBefore() || target.isOnline())) {
                    plugin.getStorageManager().debugStorage(target.getUniqueId());
                    sender.sendMessage(ChatColor.GREEN + "✓ Діагностика сховища гравця " + target.getName() + " виведена в консоль");
                } else {
                    sender.sendMessage(ChatColor.RED + "Гравця не знайдено: " + args[1]);
                }
            } else {
                sender.sendMessage(ChatColor.RED + "Використання з консолі: /storage debug <гравець>");
            }
        }
    }
    ...
    private void handleStats(CommandSender sender) {
        ...
        try {
            plugin.getStorageManager().printStatistics();
            sender.sendMessage(ChatColor.GREEN + "✓ Статистика StorageManager виведена в консоль");
            
            // Очищення неактивних GUI
            plugin.getStorageManager().cleanupInactiveGUIs();
            sender.sendMessage(ChatColor.GREEN + "✓ Очищено неактивні GUI");
        } catch (Exception e) {
            sender.sendMessage(ChatColor.RED + "Помилка при виведенні статистики: " + e.getMessage());
        }
    }
    ...
    private void sendHelp(CommandSender sender) {
        ...
        if ((sender instanceof Player && sender.hasPermission("storage.admin")) ||
            ...
            // ВИПРАВЛЕННЯ: Додано нові команди в довідку
            sender.sendMessage(ChatColor.YELLOW + "/storage debug [гравець] " + ChatColor.WHITE + "- Діагностика сховища");
            sender.sendMessage(ChatColor.YELLOW + "/storage stats " + ChatColor.WHITE + "- Статистика менеджера");
            ...
            sender.sendMessage(ChatColor.GRAY + "  storage debug Steve");
        }
    }
